...
static void FFT_Thread(void const *argument)
{
  ...
  for(;;)
  {
    ...
    {
      USBD_WCID_STREAMING_FillTxDataBuffer(&USBD_Device, 0, (uint8_t*) audio_fft_out, sizeof(float) * audio_fft_instance.init_params.FFT_len / 2);
    }
    ...
    if (nAcc == nAccTotal)
    {
      ...
      if (com_status == HS_DATALOG_USB_STARTED )
      {
        USBD_WCID_STREAMING_FillTxDataBuffer(&USBD_Device, 0, (uint8_t *)FFT_Average, sizeof(float) * audio_fft_instance.FFT_len / 2);
      }
